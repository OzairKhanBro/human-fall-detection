...
class Video:
	...
	def updateBackground(self):
		# if self.bs.method!=self.settings.bsMethod:
		# 	self.bs=bs.Bs()
	...
	def testSettings(self):
		...
		if key == ord("7"):
			if self.settings.dilationPixels>0:
				self.settings.dilationPixels -= 1
			print ("dilationPixels: " , self.settings.dilationPixels)
		if key == ord("6"):
		...
		if key == ord("a"):
			# time.sleep(1.0)
			# self.settings.movementMinimum += 1
			print (str(self.camera.get(cv2.CAP_PROP_POS_FRAMES)))
		if key == ord("1"):
			self.settings.movementMinimum  -= 1
			print ("movementMinimum: " , self.settings.movementMinimum)
		if key == ord("g"):
	...
	def compare(self):
		...
		# (_,boxes,_) = cv2.findContours(self.thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #find contours
		...
		for contour in contours:
			...
			# if self.persons.isHuman(self.frame[y:y+h, x:x+w]):
			# 	print("human ........")
			person = self.persons.addPerson(x, y, w, h,self.frame)
			color = (0, 255, 0)
			if person.alert:
				color = (0, 0, 255)
			...
			cv2.putText(self.frame, str(self.camera.get(cv2.CAP_PROP_POS_FRAMES))
										+" "+"{}".format(cv2.contourArea(contour)), (x, y+h+20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 1)
